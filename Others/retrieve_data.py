...
if __name__ == "__main__":
    ...
    table_name = 'cpu'
    start_time = '2023-04-10 23:22:40'
    end_time = '2023-04-11 07:22:40'
    ...
    query_111 = '''SELECT max(usage_user) as max_usage_user, time_bucket('300 seconds', time) AS minute
        FROM cpu
        WHERE tags_id IN (SELECT id FROM tags WHERE hostname IN ('host_32')) AND time >= '%s' AND time < '%s'
    ...
    query_581 = '''SELECT time_bucket('300 seconds', time) AS minute,
        ...
        FROM cpu
        WHERE tags_id IN (SELECT id FROM tags WHERE hostname IN ('host_9','host_43','host_45','host_19','host_39','host_35','host_15','host_41')) AND time >= '%s' AND time < '%s'
        GROUP BY minute ORDER BY minute'''%(start_time,end_time)

    query_max_all_1 = '''SELECT time_bucket('300 seconds', time) AS hour,
        ...
        FROM cpu
        WHERE tags_id IN (SELECT id FROM tags WHERE hostname IN ('host_1')) AND time >= '%s' AND time < '%s'
        GROUP BY hour ORDER BY hour'''%(start_time,end_time)
